<?php $modoEdicao = !empty($finalidade) && $finalidade->getId(); ?>
...
<div class="box-8">
    <h2 class="fonte20">
        <?php if ($modoEdicao): ?>
            ...
            Editar Finalide dos Imóveis
        <?php else: ?>
            <i class="fa-solid fa-notes-medical"></i>
            <i class="fa-solid fa-key"></i>
            Cadastrar Novo Tipo de Finalidade
        <?php endif; ?>
    </h2>
</div>
...
<form method="POST" class="box-12">
        ...
        <div class="box-12" style="background-color: transparent; color: #333;">
            <label>
                ...
                <input tabindex="1" type="text" name="descricao"
                       value="<?= $modoEdicao ? $finalidade->getDescricao() : '' ?>"
                       autofocus required>
            </label>
        </div>
    </div>
    ...
    <?php if ($modoEdicao): ?>
        <input type="hidden" name="id" value="<?= $finalidade->getId() ?>">
    <?php endif; ?>
